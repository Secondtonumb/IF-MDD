...
class PhnMonoSSLModel(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        mfa_phone_frame_ranges = batch.mfa_phone_frame_ranges
        
        phn_align = self.pool_phone_ranges_from_logits(logits=logits, phone_ranges=mfa_phone_frame_ranges, method='mean')
        import pdb; pdb.set_trace()
        
        # self.pool_phone_ranges_from_logits(logits=logits, phone_ranges=mfa_phone_frame_ranges, method='mean')

        len(mfa_phone_frame_ranges)
    ...
    def pool_phone_ranges_from_logits(
        self,
        logits,
        phone_ranges,
        method="mean",
        ...
        """
        Pool frame-level logits/features into per-phone embeddings using
        the MFA-provided phone frame ranges.

        Parameters
        ----------
        logits : Tensor
            Shape (B, T, D) - frame-level features or logits.
        phone_ranges : list
            Length-B list where each element is a list of (start, end)
            tuples (frame indices). End is treated as exclusive; values
            will be clamped to [0, T].
        method : str
            One of {'mean', 'max', 'conv'}. 'conv' applies a simple
            depthwise moving-average (kernel size = conv_kernel_size)
            before reducing across time.
        conv_kernel_size : int
            Kernel size used when method == 'conv'.

        Returns
        -------
        pooled : Tensor
            Float tensor of shape (B, R_max, D) where R_max is the
            maximum number of phone ranges in the batch. Unused slots
            are left zero.
        mask : Bool Tensor
            Shape (B, R_max) - True for valid pooled entries, False
            for padding slots.
        """
        import torch.nn.functional as F

        if logits is None:
            raise ValueError("logits must be a tensor of shape (B, T, D)")

        device = logits.device
        B, T, D = logits.size()

        # phone_ranges is expected to be a list (len B) of lists of (s,e)
        num_ranges = [len(r) if r is not None else 0 for r in phone_ranges]
        if len(num_ranges) == 0:
            return logits.new_zeros((B, 0, D)), torch.zeros((B, 0), dtype=torch.bool, device=device)

        R_max = max(num_ranges)
        pooled = logits.new_zeros((B, R_max, D))
        mask = torch.zeros((B, R_max), dtype=torch.bool, device=device)

        for i, ranges in enumerate(phone_ranges):
            if ranges is None:
                continue
            for j, rng in enumerate(ranges):
                try:
                    ...
                    # If not a 2-tuple, skip
                    continue
                s = max(0, int(s))
                e = min(int(e), T)

                if e <= s:
                    # fallback to single-frame at s if in-bounds
                    if s >= T:
                        continue
                    seg = logits[i, s : s + 1]  # [1, D]
                else:
                    seg = logits[i, s:e]  # [L, D]
                ...
                if method == "mean":
                    ...
                    # depthwise moving average conv then average across time
                    k = max(1, int(conv_kernel_size))
                    seg_t = seg.transpose(0, 1).unsqueeze(0)  # [1, D, L]
                    # depthwise weight: (D,1,k)
                    weight = logits.new_ones((D, 1, k)) / float(k)
                    pad = k // 2
                    conv_out = F.conv1d(seg_t, weight, bias=None, stride=1, padding=pad, groups=D)
                    # conv_out -> [1, D, L]
                    val = conv_out.squeeze(0).mean(dim=1)
                else:
