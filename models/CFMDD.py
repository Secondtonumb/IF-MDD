...
class CFMDD(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        # ==================== TRAINING Stage ====================
        if sb.Stage.TRAIN == stage or sb.Stage.VALID == stage or (sb.Stage.TEST == stage and hasattr(batch, 'phn_encoded_target')):
        ...
        elif sb.Stage.TEST == stage and not hasattr(batch, 'phn_encoded_target'):
            canonicals, canonical_lens = batch.phn_encoded_canonical
            canonicals_bos, canonical_lens_bos = batch.phn_encoded_canonical_bos
            canonicals_eos, canonical_lens_eos = batch.phn_encoded_canonical_eos
        ...
        return {
            ...
            "enc_out": enc_out,
            "dec_out": dec_out,
            "gated_out": p_seq_logits_gated if 'p_seq_logits_gated' in locals() else None,
            "Cano_emb": Cano_emb,
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        gated_out = predictions["gated_out"]  # [B, T_p+1, C]
        ...
        if self.hparams.decoder_target == "perceived":
            loss_dec_out = self.hparams.seq_cost(p_dec_out, perceiveds, perceived_lens_eos)
        else:
            loss_dec_out = self.hparams.seq_cost(p_dec_out, targets, target_lens_eos)
        ...
        if "dec" in self.hparams.fuse_enc_or_dec:
            ...
            loss_ga += self.hparams.ga_cost()(attn_ga, 
                                           target_lengths=(canonical_lens * Cano_emb.shape[1]).int(),
                                           input_lengths=(perceived_lens * dec_out.shape[1]).int()
                                            )
        # FUSE_attn [B, T_p, T_p]
        ...
        if self.hparams.decoder_target == "target":
            tgt_emb = self.modules.TransASR.custom_tgt_module(targets)
        else:
            tgt_emb = self.modules.TransASR.custom_tgt_module(perceiveds)
        # add tgt_pos_emb
        if (
        ...
        if getattr(self.hparams, "conpco_use_dec_pos_emb_on_audio_feat", False):
            ...
            if (
                ...
                dec_out_proj = gated_out + self.modules.TransASR.positional_encoding_decoder(gated_out)
            elif (
                self.modules.TransASR.positional_encoding_type == "fixed_abs_sine"
                or self.modules.TransASR.attention_type == "hypermixing"
            ):
        ...
        dummy_gt = torch.ones_like(perceiveds).float().to(self.device) * 2 # high score 2.0, mid score 1.0, low score 0.0
        dummy_gt = dummy_gt.masked_fill(perceiveds == 0, 0.0)
        # pdb.set_trace()
        try:
        ...
        if current_epoch % self.hparams.plot_conpco_interval == 0 and self.hparams.plot_conpco:
            ...
            if stage == sb.Stage.VALID:
                ...
                fig, ax, fig_zoom, ax_zoom = plot_phoneme_centroids_with_instances(
                    ...
                    phoneme_labels=perceiveds_eos.detach().cpu(), 
        ...
        mispro_weight = getattr(self.hparams, "mispro_loss_weight", 0)
        ga_weight = getattr(self.hparams, "ga_loss_weight", 0)
        pco_weight = getattr(self.hparams, "pco_loss_weight", 0)
        center_clap_weight = getattr(self.hparams, "center_clap_loss_weight", 0 )
        
        loss = (
            self.hparams.ctc_weight * loss_ctc
            + (1 - self.hparams.ctc_weight) * loss_dec_out
            + loss_mispro_all * mispro_weight
            + loss_ga * 10 * ga_weight
            + loss_phn_pco * pco_weight
            + loss_center_clap * center_clap_weight
        )
        # loss = loss_ctc
        ...
        if stage != sb.Stage.TRAIN:
            if current_epoch % valid_search_interval == 0 or (
                ...
                sequence_decoder_out = p_dec_out.argmax(dim=-1)  # [B, T_p+1]
                ...
                if self.hparams.decoder_target == "perceived":
                    if self.hparams.eval_with_silence == False and filtered_seq_ref is not None:
                        ...
                        self.per_metrics_seq.append(
                            ...
                            predict=sequence_decoder_out,
                            target=perceiveds,
                            predict_len=perceived_lens,
                            target_len=perceived_lens,
                            ind2lab=self.label_encoder.decode_ndim,
                        )
                else:
