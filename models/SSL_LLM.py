    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if use_prompt:
                ...
                inputs_embeds_inference = torch.cat([Z, BOS_emb], dim=1)
                inputs_embeds_inference = norm_layer(inputs_embeds_inference).to(llm_dtype)
        ...
        if stage == sb.Stage.TRAIN or stage == sb.Stage.VALID:
            ...
            if stage == sb.Stage.VALID:
                print("\n")
                print(self.hparams.LLM_tokenizer.decode(ce_targets['input_ids'][0][: ce_targets['attention_mask'][0].sum()]))
                print(self.hparams.LLM_tokenizer.decode(ce_logits.argmax(-1)[0][: ce_targets['attention_mask'][0].sum()]))
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if ce_logits is not None:
                ...
                eos_target_lens = ce_targets["input_ids"]
                # add on eos
                target_lens_eos = (target_lens  * targets.shape[-1] + 1) / (targets.shape[-1] + 1)
                ...
                self.llm_per_metrics.append(
                    ...
                    predict_len=target_lens_eos,
                    target_len=target_lens_eos,
                    ind2lab=lambda ids: [s.split() for s in self.hparams.LLM_tokenizer.batch_decode(ids, skip_special_tokens=False)]
                ) 
...
ou will be given an input speech segment. Please transcribe it into a sequence of phonemes. Output only the phoneme embedding only.
...
I will give you the speech and the reference word sequence.
