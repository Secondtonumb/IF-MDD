...
class SSL_LLM_origin_ver2(sb.Brain):
        ...
        if use_prompt and (not hasattr(self, "prompt_embed") or self.prompt_embed is None):
            ...
            if prompt_type == "soft":
                ...
                chat_structure = [
                    {
                        "role": "system",
                        "content": self.hparams.prompt_system_text,
                    },
                    {
                        "role": "user",
                        # 语音在指令之前
                        "content": (
                            f"{PLACEHOLDER}\n"
                            f"{self.hparams.prompt_user_text}"
                        ),
                    }
                ]
    ...
    def inference_batch(self, batch, max_new_tokens=100, do_sample=False, temperature=1.0, top_k=50, top_p=0.9, num_beams=1):
        ...
        ctc_argmax_tokens = None
        ctc_token_ids = None
        ctc_logits_shape = None
        if hasattr(self.modules, "ctc_lin"):
            ctc_logits = self.modules.ctc_lin(Z)
            p_ctc = self.hparams.log_softmax(ctc_logits)
            ctc_logits_shape = list(ctc_logits.shape)
            ctc_argmax_tokens = torch.argmax(p_ctc, dim=-1)
            ctc_sequence = sb.decoders.ctc_greedy_decode(
                p_ctc, wav_lens, blank_id=self.hparams.blank_index
            )
            ctc_token_ids = ctc_sequence
        ...
        generated_tokens = gen_out.sequences
        generation_logits_shape = [
            B,
            len(gen_out.scores),
            gen_out.scores[0].shape[-1] if gen_out.scores else 0,
        ]

        # Decode generated tokens to text
        generated_text = tok.batch_decode(
        ...
        return {
            ...
            "ctc_argmax_tokens": ctc_argmax_tokens,
            "ctc_logits_shape": ctc_logits_shape,
            "logits_shape": generation_logits_shape,
        }
