...
class Trans_IFMDD_ConPCO_ver2(sb.Brain):
            ...
            with torch.no_grad():
                ...
                # ==================== TEST Stage: Has GT for evaluation ====================
                    ...
                    if has_gt:
                        ...
                        # Initialize other outputs to None
                        p_ctc_logits = None
                        p_seq_logits = None
                        p_mispro_bin_logits = None
                        p_mispro_cls_logits = None
    ...
    def inference(self, test_set, test_loader_kwargs=None, max_key=None, min_key=None, output_file=None):
        ...
        results = {
            "ids": stem_ids,
            "labels": decoded_hyps,  # Decoded phoneme sequences
            "hyps_indices": all_hyps,  # Raw indices (for reference)
            "lengths": all_lengths,
            "log_probs": all_log_probs,
        }
        
        # Save results if output file is provided - CSV format with ID and label
        if output_file is not None:
            ...
            # Save predictions in CSV format: id, label
            with open(output_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(["id", "Labels"])  # Header
                for stem_id, label in zip(stem_ids, decoded_hyps):
                    writer.writerow([stem_id, label])
