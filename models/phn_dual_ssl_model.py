...
class PhnDualSSLModel(sb.Brain):
    def __init__(self, *args, patience=50, **kwargs):
        ...
        # Perceived and canonical SSL models
...
class PhnDualSSLModel_with_SimpleResidual(PhnDualSSLModel):
    ...
    def init_optimizers(self):
        ...
        # Add optimizer for residual_lin if not already present
        if hasattr(self.modules, "residual_lin") and self.modules.residual_lin is not None:
            self.adam_optimizer_diag = torch.optim.Adam(
                self.modules.residual_lin.parameters(), lr=self.hparams.lr
            )
            if self.checkpointer is not None:
                self.checkpointer.add_recoverable("adam_opt_diag", self.adam_optimizer_diag)

    def fit_batch(self, batch):
        """Fit one batch, override to do multiple updates including residual_lin."""
        if self.hparams.auto_mix_prec:
            self.pretrained_opt_class.zero_grad() # perceived_ssl
            self.pretrained_opt_cano_class.zero_grad() # canonical_ssl
            self.adam_optimizer.zero_grad()
            self.adam_optimizer_cano.zero_grad()
            self.adam_optimizer_diag.zero_grad()
            ...
            self.scaler.unscale_(self.adam_optimizer)
            self.scaler.unscale_(self.adam_optimizer_cano)
            self.scaler.unscale_(self.adam_optimizer_diag)
            ...
            if self.check_gradients(loss):
                ...
                self.scaler.step(self.adam_optimizer)
                self.scaler.step(self.adam_optimizer_cano)
                self.scaler.step(self.adam_optimizer_diag)
...
class PhnDualSSLModel_Hybrid_CTC_Attention(sb.Brain):
    def __init__(self, *args, patience=50, **kwargs):
        ...
        if self.hparams.cross_attn is not None:
            self.hparams.cross_attn.to(self.device)
        if self.hparams.attn_proj is not None:
            self.hparams.attn_proj.to(self.device)        
    ...
    def compute_forward(self, batch, stage):
        ...
        # Cross Attention: x_per attends to x_can
        # x_per: [B, T1, D], x_can: [B, T2, D]
        # self.hparams.num_heads = 8
        # self.modules.cross_attn = torch.nn.MultiheadAttention(
        #     embed_dim=x_per.size(-1), 
