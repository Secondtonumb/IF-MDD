...
class PGMDD(PhnMonoSSLModel):
    """PhnMonoSSLModel_withcanoPhnEmb_`
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #     - [!ref <enc>, !ref <ctc_lin>, !ref <CanonicalPhonemeEmbedding>, !ref <CanoTransEncoder>, !ref <ConcatTransEncoder>, !ref <mispro_head>, !ref <lab_sequence_lin>]

        if self.modules.CanonicalPhonemeEmbedding is not None:
            self.modules.CanonicalPhonemeEmbedding.to(self.device)
        if self.modules.CanoTransEncoder is not None:
            self.modules.CanoTransEncoder.to(self.device)
        if self.modules.ConcatTransEncoder is not None:
            self.modules.ConcatTransEncoder.to(self.device)
        if self.modules.mispro_head is not None:
            self.modules.mispro_head.to(self.device)
        if self.modules.lab_sequence_lin is not None:
            self.modules.lab_sequence_lin.to(self.device)
    ...
    def compute_forward(self, batch, stage):
        ...
        canonical_embeds = self.modules.CanonicalPhonemeEmbedding(canonicals) # [B, T, D]
        # Pass through LSTM
        H_Cano_phn, _ = self.modules.CanoTransEncoder(canonical_embeds) # [B, T_c, d_model]
        
        import pdb; pdb.set_trace()
        H_concat = torch.cat((feats, H_Cano_phn), dim=-1)
        
        H_concat = self.modules.ConcatTransEncoder(H_concat.transpose(1, 2)).transpose(1, 2)  # [B, T, d_model]
        
        Error_Det = self.modules.mispro_head(H_concat)  # [B, T, D]
        Error_Det = self.hparams.log_softmax(Error_Det)  # [B, T, D]
        
        Phn_pred = self.modules.lab_sequence_lin(H_concat)  # [B, T, D]
        Phn_pred = self.hparams.log_softmax(Phn_pred)  # [B, T, D]
        
        return p_ctc, Error_Det, Phn_pred, wav_lens, 
    
    def compute_objectives(self, predictions, batch, stage):
        "Given the network predictions and targets computed the NLL loss."

        Error_Det, Phn_pred, p_ctc, wav_lens = predictions
        ...
        phn_seq_labels = downsample_pool_argmax(phn_seq_labels, T_out=p_ctc.shape[1], C=self.hparams.output_neurons)
        # plt.imshow(phn_seq_labels.cpu().detach().numpy(), aspect='auto', origin='lower')
        # plt.savefig("phn_seq_labels_ds.png")
        # plt.xlabel("Time")
        # import pdb; pdb.set_trace()
        ...
        loss = loss_ctc + loss_mispro + loss_label_seq
