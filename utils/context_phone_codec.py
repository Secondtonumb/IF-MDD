...
WORD_POSITION_TRIPHONE_MODE = "word_position_triphone"
WORD_POSITION_NAMES = {
    "b": "beginning",
    "i": "interval",
    "e": "end",
...
def normalize_context_mode(mode: str | None) -> str:
    ...
    aliases = {
        ...
        "wpt": WORD_POSITION_TRIPHONE_MODE,
        "bies_triphone": WORD_POSITION_TRIPHONE_MODE,
        "position_triphone": WORD_POSITION_TRIPHONE_MODE,
        "word_position_triphone": WORD_POSITION_TRIPHONE_MODE,
        "triphone_word_position": WORD_POSITION_TRIPHONE_MODE,
    }
...
def make_context_tokens(
    ...
    word_position_triphone:
        Full left/center/right triphone context plus a B/I/E/S word-position
        suffix on the center model, e.g. ``l-aa+r@B``. This is intentionally
        untrimmed; any pruning should happen after this inventory is built.
    """
    ...
    if mode == WORD_POSITION_TRIPHONE_MODE:
        return make_word_position_triphone_tokens(phones, word_ids)
...
def make_word_position_triphone_tokens(
    phones: list[str],
    word_ids: list[int | None] | None = None,
) -> list[str]:
    """Create full B/I/E/S position-aware triphone labels."""
    labels = []
    for idx, phone in enumerate(phones):
        left = phones[idx - 1] if idx > 0 else BOUNDARY_CONTEXT
        right = phones[idx + 1] if idx + 1 < len(phones) else BOUNDARY_CONTEXT
        suffix = word_position_label_suffix(word_ids, idx)
        suffix_text = f"@{suffix}" if suffix else ""
        labels.append(f"{left}-{phone}+{right}{suffix_text}")
    return labels


def split_word_position_triphone_token(token: str) -> tuple[str, str, str] | None:
    """Return left/center/right from ``left-center+right@B`` tokens."""
    token = str(token)
...
def center_phone(token: str, mode: str) -> str:
    ...
    if mode == WORD_POSITION_UNIPHONE_MODE:
        ...
        return token
    if mode == WORD_POSITION_TRIPHONE_MODE:
        parts = split_word_position_triphone_token(token)
        return parts[1] if parts is not None else token
    raise ValueError(f"Unsupported context-phone mode: {mode}")
