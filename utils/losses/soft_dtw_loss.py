...
class SoftDTW(torch.nn.Module):
  def __init__(self, gamma=1.0, normalize=False,  cost = 'euc'):
  ...
  def calc_cross_entropy_matrix(self, x, y):
      n = x.size(1)
      m = y.size(1)
      d = x.size(2)
      x = x.unsqueeze(2).expand(-1, n, m, d)
      ...
      # Reshape x and y to be compatible with cross_entropy
      x = x.contiguous().view(-1, d)
      y = y.contiguous().view(-1, d)
      
      # Compute cross entropy for all pairs
      cross_entropy = F.cross_entropy(x, y.float(), reduction='none')
      
      # Reshape the result back to the original dimensions
      cross_entropy_matrix = cross_entropy.view(-1, n, m)
  ...
  def forward(self, x, y):
    ...
    if self.cost == 'euc':
      ...
      D_xy = self.calc_cross_entropy_matrix(x, y)
      out_xy = self.func_dtw(D_xy, self.gamma)
      result = out_xy
    else:
